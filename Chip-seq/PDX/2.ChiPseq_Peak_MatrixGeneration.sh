...
if [ -f $chip.dedup.sorted.bam ]
then
echo "Doing Zerone for "${exp}
$path_zerone/zerone -w 1000 -l -c 0.95 -0 $main/dedup_bam/$input.dedup.sorted.bam -1 $main/dedup_bam/$chip.dedup.sorted.bam > $main/peaks/Zerone/$exp.1kbp.peak.bed
